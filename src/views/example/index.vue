...
/**
 * @description:示例页面。使用基础页面模板
 * @author: 杨科
 * @date: 2020-06-03 15:58:03
 * @version: V1.0.0
 */
<template>
  <div>
    <BasePage>
      <!-- slot="header"  》》 具名插槽 指定位置 -->
      <div slot="header">
...
export default {
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          articleApi.createArticle(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          articleApi.updateArticle(tempData).then(() => {
    ...
    handleFetchPv(pv) {
      articleApi.fetchPv(pv).then(response => {
